...
(function(){
  ...
  $(".edit-space").click(function () {
    var inputTxt;
    if($('.edit-text.active').length > 0){
      inputTxt = $('.edit-text.active').val();
      $('.edit-text.active').siblings('.component-text').html(inputTxt);
    }
  ...
  jsPlumb.ready(function () {
    ...
    $(document).keypress(function (event) {
      var selectedBox = $('.active.selected-box');
      if(selectedBox.length <= 0) return;
      console.log(event.keyCode);
      if(event.keyCode&&event.keyCode === 8){
        console.log($('.active.selected-box').parent()[0].id);
        var delId = $('.active.selected-box').parent()[0].id;
    ...
    var bindClickEvent = function(newId){
      ...
      $("#"+newId).dblclick(function(e){
        e.stopPropagation();
        var text = $(this).find('.component-text').html();
        $('.edit-text').val(text);
